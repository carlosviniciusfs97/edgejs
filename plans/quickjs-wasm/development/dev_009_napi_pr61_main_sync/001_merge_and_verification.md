...
| | | Remarks |
| --- | --- | --- |
| **Status** | ▶️ | Guarded store lending with explicit-store heap allocation is implemented and propagated through the N-API, Wasmer SDK, and EdgeJS PRs; GitHub CI verification remains active. |
...
- Built and validated an intermediate direct-reentry prototype that replaced
  generic store lending with a stack-scoped pointer to the active
  `FunctionEnvMut`. It proved that the generic mechanism was not required by
  ordinary imports, but its raw reborrow across C++ was more delicate than the
  performance benefit justified and was superseded by the guarded hybrid below.
- Split guest-heap allocation into a store-free arena fast path and an explicit
  ...
  arena-only. Production store lending is now limited to the reentrant bridge
  guard plus the callback, finalizer, and foreign allocator consumers of that
  guard; ordinary N-API allocation no longer parks the store.
...
- Passed the exact EdgeJS regressions for HTTP timeout, drained-socket agent
  ...
  100 consecutive repetitions. The intermediate direct-reentry prototype's
  5,000-call `process.report.getReport()` microbenchmark averaged 490.42 ms
  versus 499.03 ms with guarded store lending (about 1.7% faster in that mixed
  workload); the final design deliberately keeps the safer lending mechanism
  on this genuinely reentrant operation.
- The intermediate direct-reentry V8+WASIX suite passed 1,675 tests. Its only failure was
  ...
  passed.
- Initially published scoped direct reentry as N-API commit `f44087e`, advanced Wasmer SDK PR
  ...
  nested store-context fix can still be useful to Wasmer independently.
- Refined that prototype into the final hybrid in N-API commit `7cd4a92`:
  `with_callback_state` parks only when a real N-API environment and explicit
  reentrant invocation guard are present; callback/finalizer trampolines and
  the V8 allocator recover that lent store with `Store::with_current`;
  ordinary imports pass their store directly to `GuestHeap::alloc_with_store`.
  This retains Wasmer's scoped, borrow-safe design without publishing a store
  on the normal N-API hot path.
- Added `allocator_hook_grows_only_with_a_lent_store`, bringing the standalone
  library suite to 45 passing tests. The final hybrid also passes the JS-backend
  compile check, locked release runner build, all focused V8+WASIX regressions,
  100 drained-socket repetitions, and the 64 MiB forced-growth check. Advanced
  Wasmer SDK PR #6956 to `154c9f89707` and EdgeJS PR #149 to `d8072e20`.
...
The direct-reentry prototype implemented the following staged simplification:
...
4. Deferred store-free `SharedMemory` growth on `sys` until Wasmer exposes it.
   ...
   ownership.
5. Removed `StoreMut::parked`/`Store::with_current` from the prototype's N-API
   hot path. The nested store-context borrow fix remained independently useful
   inside Wasmer's normal function-call machinery.

That prototype was closer to V8's explicit callback-data model: persistent callback
...
The final design keeps the strongest part of each approach. Imports with an
ordinary Rust call path use their existing store directly. Operations such as
property enumeration that cross V8 and may synchronously invoke a Proxy
`ownKeys` callback use the explicit invocation guard; the outer frame parks the
store before entering C++, and the deeper callback trampoline recovers it.
Lending cannot be deferred until the trampoline because that frame owns no
store and the outer exclusive borrow must be suspended before V8 can call back.
This confines the generic mechanism to the boundary where it is actually
needed without passing a raw `StoreMut` pointer through C++.
